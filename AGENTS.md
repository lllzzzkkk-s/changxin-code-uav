...
- Check current UGV object-target readiness before handoff; current object binding is an operator-confirmed target map, with YOLO reserved for a later perception backend: `PYTHONDONTWRITEBYTECODE=1 python3 tools/check_unit_ugv_object_target_readiness.py --target-map /tmp/unit_ugv_targets.json --object-query <object-name> --platform-id ugv_0 --write-missing-template`
- Optional future YOLO seed path: consume `ObjectDetectionSet.v1` only to prefill an unconfirmed target-map template; it still cannot authorize ROS handoff or motion: `PYTHONDONTWRITEBYTECODE=1 python3 tools/seed_unit_ugv_target_map_from_yolo_detection.py --detections <ObjectDetectionSet.v1.json> --target-map /tmp/unit_ugv_targets.json --object-query <object-name> --platform-id ugv_0 --output /tmp/changxin-unit-ugv-yolo-target-seed.json`
- Validate a local unit UGV object/target map before any ROS1 gateway call: `PYTHONDONTWRITEBYTECODE=1 python3 tools/check_unit_ugv_target_map.py --target-map /tmp/unit_ugv_targets.json --platform-id ugv_0 --require-object-queries --select-object-query <object-name> --output /tmp/changxin-unit-ugv-target-map-check.json`
...
- Start, stop, inspect, and collect signatures from the vehicle-side UGV gateway wrapper on the UGV IPC itself: `bash ugv/01-scripts/start_unit_ugv_vehicle_gateway.sh precheck --target-map ~/changxin_gateway_runtime/unit_ugv_targets.json --enable-move-base --require-move-base-server`
- From the 4060/HMI side, verify non-interactive SSH auth and operate the vehicle-side gateway without hosting the executor on 4060: `bash ugv/01-scripts/operate_unit_ugv_vehicle_gateway_ssh.sh --remote yhs@192.168.0.201 auth-check`

...
class MigrationBundleTest(unittest.TestCase):
    ...
    def test_migration_bundle_contains_required_lanes_gateway_and_commands(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("tools/check_unit_ugv_object_target_readiness.py", paths)
            self.assertIn("tools/seed_unit_ugv_target_map_from_yolo_detection.py", paths)
            self.assertIn("tools/prepare_unit_ugv_object_approach_bundle.py", paths)
            self.assertIn("tools/prepare_unit_ugv_object_approach_pipeline.py", paths)
            self.assertIn("tools/plan_unit_ugv_gateway_call.py", paths)
            self.assertIn("tools/parse_unit_ugv_ros_gateway_dry_run_response.py", paths)
            self.assertIn("tools/run_unit_ugv_ros_gateway_dry_run.py", paths)
            ...
            self.assertIn("check_unit_ugv_object_target_readiness", manifest["commands"])
            self.assertIn("seed_unit_ugv_target_map_from_yolo_detection", manifest["commands"])
            self.assertIn("check_unit_ugv_target_map", manifest["commands"])
            self.assertIn("check_unit_ugv_artifact_target_map", manifest["commands"])
            self.assertIn("prepare_unit_ugv_object_approach_bundle", manifest["commands"])
            self.assertIn("prepare_unit_ugv_object_approach_pipeline", manifest["commands"])
            self.assertIn("plan_unit_ugv_gateway_dry_run", manifest["commands"])
            self.assertIn("parse_unit_ugv_gateway_dry_run_response", manifest["commands"])
            self.assertIn("run_unit_ugv_gateway_dry_run", manifest["commands"])
            ...
            self.assertIn("check_unit_ugv_object_target_readiness.py --target-map", migration_doc)
            self.assertIn("seed_unit_ugv_target_map_from_yolo_detection.py --detections", migration_doc)
            self.assertIn("prepare_unit_ugv_object_approach_bundle.py <artifact_bundle_path>", migration_doc)
            self.assertIn("prepare_unit_ugv_object_approach_pipeline.py --profile profiles/dev_mock.env", migration_doc)
            self.assertIn("plan_unit_ugv_gateway_call.py --prep-report", migration_doc)
            self.assertIn("parse_unit_ugv_ros_gateway_dry_run_response.py --response-stdout", migration_doc)
            self.assertIn("run_unit_ugv_ros_gateway_dry_run.py --handoff-report", migration_doc)
